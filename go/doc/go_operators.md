...
关于浮点数运算，go支持FMA（fused multiply and add, 融合乘加）指令，使浮点数运算不会立刻舍入以避免丢失精度，举个例子：

```code 
// 允许使用 FMA 计算 r，因为 x*y 没有显式舍入：
r = x*y + z 
r = z; r += x*y 
t = x*y; r = t + z 
*p = x*y; r = *p + z 
...
- 字符串可比较、已排序：参考代码：
    ...
    ```
- 指针可比较：指向同一个变量或值为nil的指针相等，指向大小为0的不同变量(distinct zero-size variables)的指针，可能相等也可能不相等
- 通道可比较：使用同样的make函数构造的channel相等；值为nil的channel相等
- 接口可比较：动态类型和动态变量都相等的接口相等；值为nil的接口相等
- 非接口类型变量(s)和接口(I)可比较，要求类型s可比较且实现了接口I：当接口I的动态类型是s的类型且动态变量是s时，s和I相等
- 结构体可比较，要求它们的字段都是可比较的：当两个结构体对应的非空字段相等时，两个结构体相等
- 数组可比较，要求元素类型可比较且长度相同：对应位置元素相等的数组相等
- 比较两个拥有相同动态类型，但值不可比较的接口，panic。该规则同样适用于接口数组、包含接口字段的结构体的比较
- 切片、映射和函数不可比较，除非判等符号有一边的值为nil（即只能与nil比较）
...
- 右操作数是**一个**多值表达式，形如`x, y = f()`，例如函数调用、映射操作、类型断言等
- 左右操作数数量相等，形如`x, y = 1, 2`
...
- 空白标识符可以接收任何类型的值
- 将未定义类型的常量(untyped constant)赋值给接口类型变量或空白运算符时，常量首先被隐式转换成默认类型
