...
- 为什么所有的http请求方法都设计成了post方法？
  设计过程中，随着阅读了http请求结构、restful api风格，发现可能它们不适合直接照搬过来使用，举几个例子：
    ...
    - http 1.0 只有get/post方法，为了兼容，一些流行的库put/delete方法甚至是使用post模拟的，这大可不必
      所以我参考它们的一些优秀设计，形成了自己的接口设计风格（见附录1）

## 附录

### 1. 我的接口设计风格

- 在域名里体现api，例如`http://api.xxx.com`/`http://admin-api.xxx.com`
- 在uri里体现版本号，例如`http://api.xxx.com/v1/`
- 所有的http请求都采用post方法
- 版本号之后优先分两级（主体+行为），剩下的尽量移动到参数，例如`/v1/user/list`
    - 上述示例表示查询用户接口，至于怎么查则在参数中体现
    - 假设现在有多个查询需求，例如分页查询、查询指定id的用户、查询付费用户等，它们可以体现在uri上
      （`/v1/user/list-page`），也可以体现在参数上（参数里写上查询条件-分页，然后参数列表里包含所有查询需要的参数，
      实际调用接口和后端解析时只使用需要的参数）。
      前者胜在清晰，但是没有扩展性（假设又来一个需求：查询付费用户、根据付费总额降序、查第二页，需要写新的接口，
      而且描述性文字越来越长），后者则相反。
      对此，我的倾向是体现在参数上，接口的第一个参数叫查询条件（`condition`），是一个[]string，
      里面记录有所有查询条件，例如查询用户字段、查询结果分页/排序，后端处理时，针对每一个查询条件拼凑sql，
      最后使用拼好的sql去查。
