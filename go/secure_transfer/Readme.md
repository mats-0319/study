...
>
> 我们会提供一些平台的可执行程序，但我们建议你阅读代码后自行编译。

编译：以命令行形式使用无需外部依赖，以GUI形式使用需要安装依赖
...
- 编写技术验证demo遇到问题：最新版本的go（1.25）和ethereum（1.16）无法匹配。 因为我们没有使用以太坊的默认椭圆曲线 (secp256k1)
  。放弃go-ethereum，使用官方库编写混合加密过程（ecdh+加/解密）
